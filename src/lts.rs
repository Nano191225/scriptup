...
pub fn main(_args: Args) {
    ...
    let mc_version = get_stable_minecraft_version();
    ...
    for dependency in &mut manifest.dependencies {
        ...
        let version = match versions.iter().find(|&v| {
            !v.contains("stable")
                && !v.contains("preview")
                && !v.contains("internal")
                && !v.contains("beta")
            ...
            None => {
                eprintln!(
                    "{} No LTS versions found for module {}.",
                    style("[WARN]").yellow(),
                    module
                );
                
                let v = versions.iter().find(|&v| v.contains(&mc_version)).unwrap();
                dependency.version = extract_mc_version(v);
                v
            }
        };
...
fn get_stable_minecraft_version() -> String {
    let versions = get_versions("@minecraft/server");
    let version = versions.iter().find(|&v| v.contains("-stable"));

    if let Some(version) = version {
        extract_mc_version(version)
    } else {
        eprintln!("{} No stable versions found.", style("[ERROR]").red());
        std::process::exit(1);
    }
}
